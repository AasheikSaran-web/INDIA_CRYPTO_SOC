...
abc -liberty $LIB
clean
tee -o $RPT/${TOP}_stat.txt stat -liberty $LIB
...
YS
    ...
    if [ $RC -eq 0 ] && [ -s "$RPT/${TOP}_stat.txt" ]; then
        echo "  PASS"
        grep -E "Number of cells|Number of DFF|Chip area" "$RPT/${TOP}_stat.txt" | sed 's/^/    /'
    else
        echo "  FAIL"
        grep "ERROR" "$RPT/${TOP}.log" 2>/dev/null | head -3 | sed 's/^/    /'
    fi
}
...
} | tee "$RPT/area_summary.txt"
echo ""
echo "Results written to $RPT/ and $NET/"
